...
- URL and pasted-source analysis with one compact, complete-feeling normal result; streamed observable progress; validated fact previews; grounded Needs Attention; deterministic good/caveated/insufficient outcomes; cancellation; and a useful no-key fallback.
- Optional Extended Research that enriches the same successful result with selected costs, terms, stages, relationships, outcomes, and evidence while reusing acquired sources and never destroying the normal Overview on failure or cancellation.
...
- A feature-flagged bounded batch analyzer for at most five canonical distinct URLs with two-worker concurrency, per-item progress/cancellation, partial completion, and independent quality gating. It is hidden in the competition-facing product by default.
...
Fetched and pasted text is untrusted data. The optional OpenAI Responses API integration is server-only. Normal Analyze uses one sparse strict-output request limited to practical overview fields and grounded attention candidates; it does not emit a complete 59-field object or rich structured collections. Source metadata is supplied once and compact model evidence references use bounded source IDs plus exact excerpts. Deterministic code hydrates authoritative provenance, resolves cycle context, validates the result, and fills storage placeholders without treating unassessed deep fields as absence conclusions.

Extended Research accepts only an opaque, bounded-TTL server session created by a successful normal run. It reuses the exact acquired source contexts and validated normal card, then runs independent detailed-process and financial/outcome requests in parallel. Existing disclosed, conflicting, and not-applicable normal facts are protected from replacement; independently valid extended sections may survive another section's failure. Cancellation, timeout, or provider failure leaves the normal result intact. Every provider request sets `store: false`, uses low reasoning and low verbosity, has no automatic retry, and prohibits obeying source-page instructions or inventing unsupported claims. Every returned excerpt is deterministically matched against normalized source text; subject/recipient scope, target-program relevance, typed values, relationship/reference identity, cost/outcome semantics, and target-cycle alignment are checked before display. The application remains fully useful without `OPENAI_API_KEY`.

Automated output is always a draft. Streamed progress may expose only observable acquisition events, completed research milestones, deterministically retained preview facts, validation counts, and safe quality events; hidden reasoning and speculative candidate claims are never shown. Needs Attention explanations must survive deterministic claim/status/source-reference grounding. Normal Analyze retains at most three decision-important items; Extended Research may retain five. The compact quality gate judges identity, source acquisition, practical decision-area coverage, retained evidence, and catastrophic failure rather than penalizing absent deep ledgers. An insufficient result is suppressed by default but retained for an explicit, persistent `Incomplete result · quality gate overridden` view that performs no retry.

Eligible deterministic quality failures may be cached by canonical URL and analyzer version. Same-browser unchanged retries reuse the retained incomplete result instead of spending again; a materially different URL or added pasted text may run normally. A durable cross-user hit performs a bounded submitted-page fingerprint check when available and spends no model tokens if unchanged. Provider, cancellation, temporary network, and internal failures are never durable entries. The server is authoritative for suppression: `ANALYSIS_FAILURE_CACHE_BYPASS_HOSTS` disables durable cache reads/writes, local retry suppression, and same-browser cooldown for configured hosts, but is never passed into acquisition, prompts, validation, quality thresholds, projections, or result interpretation.
