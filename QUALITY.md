...
- Extended Research continuation state is intentionally instance-local and expires after 30 minutes. That is appropriate for the competition demo, but a multi-instance deployment can lose the continuation if routing changes; the normal result remains intact and the UI reports the extension failure. Durable continuation would be a later deployment architecture decision, not a reason to trust browser-supplied card state.
